      ...
      <el-aside width="400px" class="aside_field">
        <el-affix :offset="1" target=".aside_field">
          ...
          <div class="card_field">
            <div
              class="card"
              v-for="news in this.documents_relevant.slice(0, 3)"
              :key="news"
            >
      ...
      <el-main>
        <div class="content_field">
          ...
          <div class="content_main" v-loading.fullscreen.lock="oss_loading">
            <p v-html="this.documents_info.NewsFlashContent"></p>
          </div>
        </div>
      </el-main>
    </el-container>
...
export default {
  ...
  methods: {
    getParams() {
      this.documents_id = this.$route.query.documents_id;
      //在此处向服务器请求数据，给所需变量重新赋值
      this.oss_loading=true;
      axios
        .get("spring/admin/newsflash/single" + "?newsflash_id=" + this.documents_id)
        .then((res) => {
          this.documents_info = res.data.data;
          this.new_tags = [].concat(res.data.data.NewsFlashTag.split("-"));
          this.documents_info.NewsFlashDate = this.documents_info.NewsFlashDate.substring(
            0,
            this.documents_info.NewsFlashDate.indexOf(" ")
          );
          const xhrFile = new XMLHttpRequest();
          console.log("开始解析oss");
          xhrFile.open("GET", this.documents_info.NewsFlashContent, true);
          xhrFile.send();
          xhrFile.onload = () => {
            //res.data.data.blog_content=xhrFile.response;
            this.documents_info.NewsFlashContent = xhrFile.response;
            console.log("oss解析完成");
  ...
  created() {
    ...
    axios
      .get("spring/admin/newsflash/all")
      .then((res) => {
        this.documents_relevant = [].concat(res.data.data.newsflashs.filter(
          (news) => news.NewsFlashId != this.documents_id
        ));
      })
      .catch((errMsg) => {
        console.log(errMsg);
